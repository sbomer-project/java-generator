...
The **Java Generator Service** is a specialized microservice within the SBOMer architecture responsible for executing SBOM generation requests for Java projects using [CycloneDX Maven Plugin](https://github.com/CycloneDX/cyclonedx-maven-plugin) and [Domino](https://github.com/quarkusio/quarkus-platform-bom-generator/tree/main/domino).
...
* **`KafkaRequestConsumer`:** Listens to the `generation.created` topic. If the request matches `sbomer.generator.name=java-generator`, it queues it for execution.
...
|:----------------------------------------------| :--- |:--------------------------------|
| `sbomer.generator.name`                       | The name used to filter incoming Kafka events. | `java`                          |
| `sbomer.generator.cdx-maven-plugin.task-name` | The Tekton Task name to instantiate. | `generator-cdx-maven-plugin`    |
| `sbomer.generator.domino.task-name`           | The Tekton Task name to instantiate. | `generator-domino`              |
...
- Builds component images (java-generator, java-agent) and puts them into minikube

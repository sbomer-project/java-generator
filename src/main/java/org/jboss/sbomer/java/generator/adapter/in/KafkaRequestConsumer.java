...
import static org.jboss.sbomer.java.generator.core.ApplicationConstants.COMPONENT_NAME;
        ...
        try {
            ...
            if (isMyGenerator(event)) {
                ...
                orchestrator.acceptRequest(
                        ...
                        event.getData().getRecipe().getGenerator().getOptions(),
                        event.getData().getGenerationRequest().getHandlerProvidedOptions(),
                        traceParent
                );
            }
        } catch (Exception e) {

...
LOCAL_CHART_PATH="helm/java-generator-chart"
...
echo "--- Building local java-agent image inside of Minikube ---"
# Will have local image java-agent:latest in minikube
bash ./hack/build-local-java-agent-into-minikube.sh

echo "--- Building local java-generator image inside of Minikube ---"
# Will have local image java-generator:latest in minikube
bash ./hack/build-local-java-generator-into-minikube.sh
...
target_name = 'java-generator-chart'
...
for line in lines:
    ...
    # If we are inside the java-generator-chart block, replace values
    if in_block:
...
helm upgrade --install sbomer-release "./$PLATFORM_DIR" \
    ...
    --set java-generator-chart.image.repository=localhost/java-generator \
    --set java-generator-chart.image.tag=latest \
    --set java-generator-chart.image.pullPolicy=Never \
    --set java-generator-chart.task.agent.image=localhost/java-agent \
    --set java-generator-chart.task.agent.tag=latest \
    --set java-generator-chart.task.agent.pullPolicy=Never

echo "--- Forcing Rolling Restart to pick up new local image ---"
# We ignore "not found" errors in case it's the very first install
kubectl rollout restart deployment -n $NAMESPACE -l app.kubernetes.io/name=java-generator-chart || true
